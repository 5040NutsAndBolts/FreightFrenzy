...
public class HelperMethods {
    ...
    public static void setMotorVelocity(DcMotorEx motor, double velocity)
    {

        double maxVelocity = 2600;
        double divide=150;
        double holdVelocity=velocity/maxVelocity/1.5;
        if(Math.abs(velocity)<Math.abs(motor.getVelocity()))
        {
            holdVelocity/=1.5;
            divide = 200;
        }
        motor.setPower((velocity-motor.getVelocity())/divide+(velocity)/Math.abs(velocity)*holdVelocity);
